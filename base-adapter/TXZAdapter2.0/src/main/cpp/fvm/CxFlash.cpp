...
//#define MAX_DEV 256
//char g_dev[MAX_DEV] = "/sys/devices/platform/zwattr/denoise_rst";
...
char *reset(int a)
{
	printf("echo %d > /sys/devices/platform/zwattr/denoise_rst\n", a);
	//return 0;
...
char s[100];
char *rest(const char *reset_address,int a)
{
    char address[100] = {0};
    if (reset_address == NULL){
        if (a==1){
            strcat(address,"echo 1 > /sys/devices/platform/unisound/uni_rst");
        } else{
            strcat(address,"echo 0 > /sys/devices/platform/unisound/uni_rst");
        }
    } else {
        if (a==1){
            strcat(address,"echo 1 > ");
            strcat(address,reset_address);
        } else {
            strcat(address,"echo 0 > ");
            strcat(address,reset_address);
        }
    }
    LOGD("CxFlah reset:: 111111111");
    LOGD("CxFlash rest:: %s",address);
	if(a==1)
	{
		strcpy(s, address);
        //free(address);
...
int download_boot_loader(const unsigned char *  loader_data, unsigned int num_byte,const char* reset_address,int mode)
{
	int  ReadErr = ERRNO_NOERR;
	...
	unsigned int   time_out_loop = TIME_OUT_MS/POLLING_INTERVAL_MS;
	int err_no = ERRNO_NOERR;
	printf("******download_boot_loader******\n");
    LOGD("CxFlash download_boot_loader ******download_boot_loader******");
	LOG((DBG_INFO "Waiting. Please reset the board\n"));
    LOGD("CxFlash download_boot_loader Waiting. Please reset the board");
...
/***
			...
			fd = open("/dev/gpiod20", O_WRONLY);
			if (fd < 0) 
			{
//		    	fprintf(stderr, "failed to open conexant_reset: %s\n", gpio_name);
		    }
...
***/
            if (reset_address == NULL)
                reset_address = "/sys/devices/platform/unisound/uni_rst";
            LOGD("CxFlash download_boot_loader echo mode::%d",mode);
            if (mode == 0) {
                //"echo 1 > /sys/devices/platform/unisound/uni_rst"
                LOG((DBG_INFO "please echo 0 > %s\n",reset_address));
                LOGD("CxFlash download_boot_loader please echo 0 > %s",reset_address);
//			system("echo 0 > /sys/devices/platform/zwattr/denoise_rst");
                system(rest(reset_address,0));
                sys_mdelay(RESET_INTERVAL_MS);
                LOG((DBG_INFO "please echo 1 > %s\n",reset_address));
                LOGD("CxFlash download_boot_loader please echo 1 > %s",reset_address);
//			system("echo 1 > /sys/devices/platform/zwattr/denoise_rst");
                system(rest(reset_address,1));
            } else {
                LOGD("CxFlash download_boot_loader file %s",reset_address);
                FILE *fp = NULL;
                char *reset_flag;

                reset_flag = "0";
                //fp = fopen("/sys/class/fvm_dev/fvm_reset", "w");/sys/devices/platform/unisound/uni_rst
                fp = fopen(reset_address, "w");
                fwrite(reset_flag, sizeof(reset_flag), strlen(reset_flag), fp);
                fflush(fp);
                LOGD("CxFlash download_boot_loader w 0 > %s",reset_address);
                sys_mdelay(RESET_INTERVAL_MS);
                reset_flag = "1";
                fwrite(reset_flag, sizeof(reset_flag), strlen(reset_flag), fp);
                fflush(fp);
                LOGD("CxFlash download_boot_loader w 1 > %s",reset_address);
                fclose(fp);
            }
	...
	*/
	...
	printf("val=0x%x\n",val);
    LOGD("CxFlash download_boot_loader val=0x%x",val);
	//LOG((DBG_ERROR "val=0x%x",val);
	if( time_out_loop == 0)
	{
		//LOG((DBG_ERROR "val=0x%x",val);
		printf("***test01***\n");
        LOGW("CxFlash download_boot_loader ***test01***");
		LOG((DBG_ERROR " ******aborting. timeout wating for device after %d ms\n",
			(g_SetResetPinPtr?TIME_OUT_MS:MANUAL_TIME_OUT_MS)*POLLING_INTERVAL_MS));
        LOGW("CxFlash download_boot_loader ******aborting. timeout wating for device after %d ms",
             (g_SetResetPinPtr?TIME_OUT_MS:MANUAL_TIME_OUT_MS)*POLLING_INTERVAL_MS);
	...
	*/
	...
	if( g_bIsRenoDev )
	{
		...
		printf("err_no=%d\n", err_no);
        LOGD("CxFlash download_boot_loader err_no=%d",err_no);
	}
	else
	{
...
int verify_firmware_version(const char *reset_address) {
...
//		    	fprintf(stderr, "failed to open conexant_reset: %s\n", gpio_name);
			...
			reset_flag = "0";
            //fp = fopen("/sys/class/fvm_dev/fvm_reset", "w");/sys/devices/platform/unisound/uni_rst
            fp = fopen("/sys/devices/platform/unisound/uni_rst", "w");
			fwrite(reset_flag, sizeof(reset_flag), strlen(reset_flag), fp);
			fflush(fp);
    ...
    int mode)
{
	...
	do{
		...
		if( NULL == pBuffer )
		{
            ...
            LOGE("CxFlash DownloadFW The buffer pointer is not set");
			break;
		}
			...
			NULL == g_I2cWriteMemPtr )
		{
			err_no = -ERRNO_I2CFUN_NOT_SET;
			LOG((DBG_ERROR "i2C function is not set.\n"));
            LOGE("CxFlash DownloadFW i2c function is not set.");
			break;
		}
		...
		else
		{
			if ((header = ParseImageHeader(image_data,&image_size,&g_is_partial_img))==NULL)
			{
                ...
                LOGE("CxFlash DownloadFW SFS image invalid.");
				break;
			}
		}
		...
		if( g_update_mode > SFS_UPDATE_BOTH )
		{
            ...
            LOGE("CxFlash DownloadFW Parameter is invalid.");
			break;
		}
		...
		if (g_bBootTimeUpgrade)
		{
			...
			if (loader_data == NULL)
			{
				if (header->iflash ==(dword)BLKNONE)
				{
                    ...
                    LOGE("CxFlash DownloadFW didn't find a valid iflash bootloader in image. aborting.");
					break;
				}
				else
				{
			...
			else
			{
				if (g_bBootTimeUpgrade){
                    ...
                    LOGE("CxFlash DownloadFW didn't find a valid bootloader magic number. aborting.");
					break;
				}
			}
		}
        ...
        LOGD("CxFlash DownloadFW Device Type          : %s ",g_bIsRenoDev? "Reno":"Hudson");
		LOG((DBG_INFO "I2C Slave Address    : %02xh\n", g_bChipAddress));
        LOGD("CxFlash DownloadFW I2C Slave Address    : %02xh",g_bChipAddress);
		LOG((DBG_INFO "    Max burst write  : %d bytes\n", g_cbMaxI2cWrite));
        LOGD("CxFlash DownloadFW    Max burst write  : %d bytes",g_cbMaxI2cWrite);
		LOG((DBG_INFO "    Max burst read   : %d bytes\n", g_cbMaxI2cRead));
        LOGD("CxFlash DownloadFW    Max burst read   : %d bytes",g_cbMaxI2cRead);
		LOG((DBG_INFO "Endianness           : %s-Endian \n", little_endian? "Little":"Big"));
        LOGD("CxFlash DownloadFW Endianness           : %s-Endian ",little_endian? "Little":"Big");
        ...
        LOGD("CxFlash DownloadFW SFS version          : %d.%d.%d.%d ",(uint8_t) (g_sfs_version>>24),
             (uint8_t) (g_sfs_version>>16),(uint8_t) (g_sfs_version>>8), (uint8_t) (g_sfs_version));

		if( ((uint32_t) -1) != g_firmware_version[0]) {
            LOG((DBG_INFO "Firmware version     : %d.%d.%d.%d \n", g_firmware_version[0],
                    g_firmware_version[1],g_firmware_version[2],g_firmware_version[3]));
            LOGD("CxFlash DownloadFW Firmware version     : %d.%d.%d.%d ",g_firmware_version[0],
                 g_firmware_version[1],g_firmware_version[2],g_firmware_version[3]);
        }
		...
		if (g_bBootTimeUpgrade)
		{
            ...
            LOGD("CxFlash DownloadFW Boot loader size     : %xh bytes",loader_size);
			LOG((DBG_INFO "Image size           : %xh bytes\n", image_size));
            LOGD("CxFlash DownloadFW Image size           : %xh bytes",image_size);
		}

		if(!g_legacy_device){  
			LOG((DBG_INFO "Unpadded image       : %s \n", g_is_partial_img?"Yes":"No"));
            LOGD("CxFlash DownloadFW Unpadded image       : %s ",g_is_partial_img?"Yes":"No");
			LOG((DBG_INFO "Is Dual Image        : %s \n", g_is_dual_img?"Yes":"No"));
            LOGD("CxFlash DownloadFW Is Dual Image        : %s ", g_is_dual_img?"Yes":"No");
			LOG((DBG_INFO "Update partition     : %s \n", update_mode_str[g_update_mode] ));
            LOGD("CxFlash DownloadFW Update partition     : %s ", update_mode_str[g_update_mode] );
		}
		LOG((DBG_INFO "\n"));
		...
		if (g_bBootTimeUpgrade==1)
		{
            ...
            LOGD("CxFlash DownloadFW Downloading boot loader");
			err_no =  download_boot_loader(loader_data,loader_size,reset_address,mode);

			if( err_no == 0) 
			{
				LOG((DBG_INFO "\nBootloader transfer completed successfully\n"));
                LOGD("CxFlash DownloadFW Bootloader transfer completed successfully");
			}
			else
			{
				LOG((DBG_ERROR "\nFailed to download boot loader\n"));
                LOGE("CxFlash DownloadFw Failed to download boot loader");
				break;
			}
		}
		else
		{
			...
			Command cmd = {0,CONTROL_APP_FW_UPGD,0,static_cast<unsigned int>APP_ID('C','T','R','L')};
		...
		LOG((DBG_INFO "\nDownloading image\n"));
        LOGD("CxFlash DownloadFW Downloading image");
		err_no = download_image(image_data,image_size);
		if(err_no  == 0) 
		{
			LOG((DBG_INFO "\nImage transfer completed successfully!\n"));
            LOGD("CxFlash DownloadFW Image transfer completed successfully!");
		}
		else
		{
			LOG((DBG_ERROR "\nFailed to download Image, error code = %d\n", err_no));
            LOGE("CxFlash DownloadFW Failed to download Image, error code = %d",err_no);
			break;
		}

		/*Check if the firmware version is correct or not*/
		err_no = verify_firmware_version(reset_address);
	}while(0);
	// ShowProgress(0,2,0,0);
    LOGD("CxdFlash DownloadFW err_no::%d",err_no);
	return err_no;
}
...
int SendCmdV2048 (Command2048 *cmd)
{
	int elapsed_ms = 0;
	int num_32b_words = cmd->num_32b_words;
	unsigned int *i2c_data = (unsigned int *)cmd;
	int size = num_32b_words + 2;
	int err_no;
	cmd->num_32b_words = (cmd->command_id&CMD_GET(0)) ? 2048 : num_32b_words;
